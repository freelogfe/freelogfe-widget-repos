...
export default {
  ...
  methods: {
    ...
    fetchModuleI18nDataByModuleName(module) {
      return window.FreelogApp.QI.fetch(`//i18n-ts.testfreelog.com/v1/i18nRepository/data?targetPath=${encodeURIComponent(module.path)}&pathType=2&repositoryName=${this.repositoryName}`)
        .then(res => res.json())
        .then(res => {
          if (res.errcode === 0) {
