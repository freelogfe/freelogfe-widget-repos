...
function wraplineIntoNode(line) {
  ...
  if (lastIndex === -1) {
    ...
    let releaseId = value.substring(lastIndex + 1).trim()
    let reg = /<(\w+)>/
    if (reg.test(releaseId)) {
      releaseId = reg.exec(releaseId)[1]
      node.nested = true
    }
...
function catalogParser(text, options) {
  ...
  const catalogList = linesInfo.map(item => {
    return wraplineIntoNode(item)
  }).filter(item => item.releaseId)
  return {
    catalogList,
    catalogTreeData: transformLinesToTree(linesInfo)
  }
}
